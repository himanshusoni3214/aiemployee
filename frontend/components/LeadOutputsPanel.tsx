...
export function LeadOutputsPanel({ outputs, rows }: { outputs: LeadOutput[]; rows: Record<string, unknown>[] }) {
  ...
  if (!latest) return <div className="text-xs text-zinc-500">No generated lead/report files found yet.</div>;
  ...
  return (
    <div className="mt-3 rounded border border-zinc-800 p-3" data-voryx-lead-outputs>
      <div className="grid gap-1 text-xs text-zinc-400">
        <div>Latest file: <span className="text-zinc-200">{latest.file_name || latest.path}</span></div>
        <div>Path: <span className="text-zinc-200">{latest.path}</span></div>
        <div>Type: {latest.kind || 'csv'} / Rows: {latest.row_count} / Modified: {latest.modified_at || latest.generated_at}</div>
      </div>
      <div className="mt-2 flex flex-wrap gap-2">
        <a className="btn-secondary text-xs" href={latest.download_url} data-voryx-download-csv>{latest.kind === 'report' ? 'Download Report' : 'Download CSV'}</a>
      ...
      {latest.kind !== 'report' && rows.length ? (
